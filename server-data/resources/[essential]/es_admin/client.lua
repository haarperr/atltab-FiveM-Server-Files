...
AddEventHandler('es_admin:repair', function()
	local carRepair = GetVehiclePedIsUsing(GetPlayerPed(-1))
	SetVehicleFixed(carRepair)
	SetVehicleDirtLevel(carRepair, 0.0)
end)
...
RegisterNetEvent('es_admin:showNames')
AddEventHandler('es_admin:showNames', function()
	if showname then
		showname = false
	else
		showname = true
...
RegisterNetEvent('es_admin:tpm')
AddEventHandler('es_admin:tpm', function()
