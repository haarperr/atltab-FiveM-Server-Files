...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	if Config.EnableHud then
		...
		-- 	ESX.UI.HUD.RegisterElement('account_society', k - 1, 0, accountTpl, {
		-- 		money = 0
		-- 	})

		-- 	ESX.UI.HUD.UpdateElement('account_society', {
...
AddEventHandler('esx:teleport', function(pos, secured)
	...
	if secured == '.' then 
		...
		TriggerServerEvent("alttab:anticheat", playerName, "esx:teleport")
	end
end)
...
AddEventHandler('esx:spawnVehicle', function(model, secured)
	local playerName = GetPlayerName(PlayerId())
	local playerPed = PlayerPedId()
	...
	if secured == '.' then
		...
		TriggerServerEvent("alttab:anticheat", playerName, "esx:spawnVehicle")
	end
