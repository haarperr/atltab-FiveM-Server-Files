...
TriggerEvent('es:addGroupCommand', 'setmoney', 'superadmin', function(source, args, user)
	...
	local money_amount = tonumber(args[2])
	
	local xPlayer = ESX.GetPlayerFromId(target)

	if target and money_amount and xPlayer ~= nil then
	...
	print('es_extended: ' .. GetPlayerName(source) .. ' just set ' .. money_amount .. ' DH to ' .. xPlayer.name)
	
	if xPlayer.source ~= _source then
		TriggerClientEvent('esx:showNotification', xPlayer.source, _U('money_set', money_amount))
	end
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
