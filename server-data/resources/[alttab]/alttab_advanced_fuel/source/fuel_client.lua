...
function ManageFuelUsage(vehicle)
	...
	if IsVehicleEngineOn(vehicle) then
		SetFuel(vehicle, GetVehicleFuelLevel(vehicle) - Config.FuelUsage[Round(GetVehicleCurrentRpm(vehicle), 1)] * (Config.Classes[GetVehicleClass(vehicle)] or 0.5) / 10)
	end
end
...
AddEventHandler('alttab_advanced_fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	if pumpObject then
		TriggerServerEvent('alttab_advanced_fuel:pay', currentCost)
	end
...
AddEventHandler('alttab_advanced_fuel:refuelFromPump', function(pumpObject, ped, vehicle)
	...
	TriggerEvent('alttab_advanced_fuel:startFuelUpTick', pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if pumpObject then
			...
			if Config.UseESX then
				extraString = "\n" .. Config.Strings.TotalCost .. ": ~g~" .. Round(currentCost, 1) .. " DH"
			end
						...
						if GetVehicleFuelLevel(vehicle) < 95 and canFuel then
							if currentCash > 0 then
								...
								if IsControlJustReleased(0, 38) then
									...
									TriggerEvent('alttab_advanced_fuel:refuelFromPump', isNearPump, ped, vehicle)
									LoadAnimDict("timetable@gardener@filling_can")
								end
							else
					...
					if currentCash >= Config.JerryCanCost then
						if not HasPedGotWeapon(ped, 883325847) then
							...
							if IsControlJustReleased(0, 38) then
								...
								TriggerServerEvent('alttab_advanced_fuel:pay', Config.JerryCanCost)
							...
							if Config.UseESX then
								...
								if refillCost > 0 then
									if currentCash >= refillCost then
										...
										if IsControlJustReleased(0, 38) then
											TriggerServerEvent('alttab_advanced_fuel:pay', refillCost)
...
function CreateBlip(coords)
	...
	AddTextComponentString("Station d'essence")
