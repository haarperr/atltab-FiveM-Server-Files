...
RegisterNetEvent("esx_inventoryhud:openPlayerInventoryDead")
AddEventHandler("esx_inventoryhud:openPlayerInventoryDead", function(target, playerName)
    local target, closestDistance = ESX.Game.GetClosestPlayer()
    if IsPlayerDead(target) then 
        if target ~= -1 and closestDistance <= 2.5 then
            targetPlayer = target
            targetPlayerName = playerName
            setPlayerInventoryData()
            openPlayerInventory()
        ...
        exports['mythic_notify']:SendAlert('error', 'mn nytk azbi?') 
    end
end)
