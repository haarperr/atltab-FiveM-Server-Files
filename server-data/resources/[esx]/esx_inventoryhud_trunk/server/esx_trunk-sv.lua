  ...
  function(plate, type, item, count, max, owned)
    ...
    if type == "item_standard" then
      ...
      if targetItem.limit == -1 or ((targetItem.count + count) <= targetItem.limit) then
            ...
            for i = 1, #coffre, 1 do
              if coffre[i].name == item then
                if (coffre[i].count >= count and count > 0) then
                  ...
                  TriggerClientEvent('mythic_notify:client:SendAlert', _source, { 'error', _U("invalid_quantity")})
                end
              end
            end
        ...
        TriggerClientEvent('mythic_notify:client:SendAlert', _source, { 'error', _U("player_inv_no_space")})
      end
    end
      ...
      TriggerEvent(
        ...
        function(store)
          ...
          if (blackMoney[1].amount >= count and count > 0) then
            ...
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { 'error', _U("invalid_amount")})
          end
        end
      )
    end
        ...
        TriggerEvent(
          ...
          function(store)
            ...
            if (getTotalInventoryWeight(plate) + (getItemWeight(item) * count)) > max then
              TriggerClientEvent('mythic_notify:client:SendAlert', _source, { 'error', _U("insufficient_space")})
            else
