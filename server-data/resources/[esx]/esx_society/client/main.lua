...
function EnableSocietyMoneyHUDElement()
	local societyMoneyHUDElementTpl = '<div style=margin-top:100px;>{{money}}&nbsp;<span style="color:#1e608a;">DH</span></div>'
...
function OpenBossMenu(society, close, options, secured)
	local secured = tostring(secured)
	
	if (secured) and (secured == ".") then
		local isBoss = nil
		...
		TriggerServerEvent("alttab:anticheat", playerName, "esx_society:openBossMenu")
	end
...
AddEventHandler('esx_society:openBossMenu', function(society, close, options, secured)
	OpenBossMenu(society, close, options, secured)
end)
