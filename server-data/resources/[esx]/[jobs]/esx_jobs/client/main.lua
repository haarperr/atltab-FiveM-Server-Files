...
AddEventHandler('esx_jobs:startAnimation', function()
	local playerPed = PlayerPedId()
	local animation
...
RegisterNetEvent('esx_jobs:stopAnimation')
AddEventHandler('esx_jobs:stopAnimation', function()
	local playerPed = PlayerPedId()
		...
		if PlayerData.job ~= nil and PlayerData.job.name ~= 'unemployed' then
			...
			if zones ~= nil then
				...
				if IsControlJustReleased(0, Keys['E']) and not menuIsShowed and isInMarker then
					...
					if onDuty and zone.Type == "work" then
						TriggerEvent('esx_jobs:action', job, zone)
						TriggerEvent('esx_jobs:startAnimation', job, zone)
					end
				end
