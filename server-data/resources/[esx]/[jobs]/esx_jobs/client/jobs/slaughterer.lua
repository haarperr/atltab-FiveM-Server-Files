Config.Jobs.slaughterer = {

	BlipInfos = {
		Sprite = 256,
		Color = 5
	},

	Vehicles = {

		Truck = {
			Spawner = 1,
			Hash = "benson",
			Trailer = "none",
			HasCaution = true
		}
	},

	Zones = {

		CloakRoom = {
			-- Pos = {x = -1070.58, y = -2003.00, z = 15.78},
			-- Pos = {x = -603.80, y = -870.84, z = 24.5},
			Pos = {x = -74.80, y = 6250.84, z = 31.09},
			Size = {x = 1.0, y = 1.0, z = 1.0},
			Color = {r = 204, g = 204, b = 0},
			Marker = 20,
			Blip = true,
			Name = _U("s_slaughter_locker"),
			Type = "cloakroom",
			Hint = _U("cloak_change")
		},

		AliveChicken_1 = {
			Pos = {x = -62.50, y = 6241.46, z = 31.09},
			Size = {x = 1.0, y = 1.0, z = 1.0},
			Color = {r = 204, g = 204, b = 0},
			Marker = 20,
			Blip = true,
			Name = _U("s_hen"),
			Type = "work",
			Item = {
				{
					name = _U("s_alive_chicken"),
					db_name = "alive_chicken",
					time = 2500,
					max = 20,
					add = 1,
					remove = 1,
					requires = "nothing",
					requires_name = "Nothing",
					drop = 100
				}
			},
			Hint = _U("s_catch_hen")
		},
		AliveChicken_2 = {
			Pos = {x = -66.23, y = 6236.69, z = 31.09},
			Size = {x = 1.0, y = 1.0, z = 1.0},
			Color = {r = 204, g = 204, b = 0},
			Marker = 20,
			Blip = false,
			Name = _U("s_hen"),
			Type = "work",
			Item = {
				{
					name = _U("s_alive_chicken"),
					db_name = "alive_chicken",
					time = 2500,
					max = 20,
					add = 1,
					remove = 1,
					requires = "nothing",
					requires_name = "Nothing",
					drop = 100
				}
			},
			Hint = _U("s_catch_hen")
		},

		SlaughterHouse = {
			Pos = {x = -77.80, y = 6229.06, z = 31.09},
			Size = {x = 1.0, y = 1.0, z = 1.0},
			Color = {r = 204, g = 204, b = 0},
			Marker = 20,
			Blip = false,
			Name = _U("s_slaughtered"),
			Type = "work",
			Item = {
				{
					name = _U("s_slaughtered_chicken"),
					db_name = "slaughtered_chicken",
					time = 3000,
					max = 20,
					add = 1,
					remove = 1,
					requires = "alive_chicken",
					requires_name = _U("s_alive_chicken"),
					drop = 100
				}
			},
			Hint = _U("s_chop_animal")
		},

		Packaging = {
			Pos = {x = -101.97, y = 6208.79, z = 31.02},
			Size = {x = 1.0, y = 1.0, z = 1.0},
			Color = {r = 204, g = 204, b = 0},
			Marker = 20,
			Blip = false,
			Name = _U("s_package"),
			Type = "work",
			Item = {
				{
					name = _U("s_packagechicken"),
					db_name = "packaged_chicken",
					time = 2000,
					max = 100,
					add = 5,
					remove = 1,
					requires = "slaughtered_chicken",
					requires_name = _U("s_unpackaged"),
					drop = 100
				}
			},
			Hint = _U("s_unpackaged_button")
		},

		Delivery = {
			Pos = {x = -596.15, y = -889.32, z = 24.50},
			Color = {r = 204, g = 204, b = 0},
			Size = {x = 15.0, y = 15.0, z = 1.0},
			Marker = -1,
			Blip = true,
			Name = _U("delivery_point"),
			Type = "delivery",
			Spawner = 1,
			Item = {
				{
					name = _U("delivery"),
					time = 1000,
					remove = 1,
					max = 100, -- if not present, probably an error at itemQtty >= item.max in esx_jobs_sv.lua
					price = 5,
					requires = "packaged_chicken",
					requires_name = _U("s_packagechicken"),
					drop = 100
				}
			},
			Hint = _U("s_deliver")
		}
	}
}
