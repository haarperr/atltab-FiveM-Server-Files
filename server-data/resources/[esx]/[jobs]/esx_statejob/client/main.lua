			...
			if Config.MaxInService ~= -1 then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'state')

						TriggerServerEvent('esx_service:disableService', 'state')
						TriggerEvent('esx_statejob:updateBlip')
...
function OpenVehicleSpawnerMenu(type, station, part, partNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			if type == 'car' then
				shopCoords = Config.StateStations[station].Vehicles[partNum].InsideShop
			...
			elseif type == 'helicopter' then
				shopCoords = Config.StateStations[station].Helicopters[partNum].InsideShop
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('esx_statejob:storeNearbyVehicle', function(storeSuccess, foundNum)
...
function GetAvailableVehicleSpawnPoint(station, part, partNum)
	local spawnPoints = Config.StateStations[station][part][partNum].SpawnPoints
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_statejob:buyJobVehicle', function (bought)
					if bought then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'body_search' then
						TriggerServerEvent('esx_statejob:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
					...
					elseif action == 'handcuff' then
						...
						if distance <= 2.0 then
							TriggerServerEvent('esx_statejob:requestarrest', target_id, playerheading, playerCoords, playerlocation)
						else
					...
					elseif action == 'uncuff' then
						...
						local target_id = GetPlayerServerId(target)
						if distance <= 2.0 then
							TriggerServerEvent('esx_statejob:requestrelease', target_id, playerheading, playerCoords, playerlocation)
						else
...
function OpenVehicleInfosMenu(vehicleData)
	ESX.TriggerServerCallback('esx_statejob:getVehicleInfos', function(retrivedInfo)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if PlayerData.job and PlayerData.job.name == 'state' and PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_statejob:hasEnteredEntityZone', function(entity)
	...
	if PlayerData.job and PlayerData.job.name == 'state' and IsPedOnFoot(playerPed) then
...
RegisterNetEvent('esx_statejob:unrestrain')
...
AddEventHandler('esx_statejob:drag', function(copId)
...
AddEventHandler('esx_statejob:OutVehicle', function()
	local playerPed = PlayerPedId()
...
RegisterNetEvent('esx_statejob:getarrested')		
AddEventHandler('esx_statejob:getarrested', function(playerheading, playercoords, playerlocation)		
	...
	Citizen.Wait(3760)		
	isHandcuffed = true		
	TriggerEvent('esx_statejob:handcuff')		
	loadanimdict('mp_arresting')		
	TaskPlayAnim(GetPlayerPed(-1), 'mp_arresting', 'idle', 8.0, -8, -1, 49, 0.0, false, false, false)		
end)		
RegisterNetEvent('esx_statejob:doarrested')		
AddEventHandler('esx_statejob:doarrested', function()		
	Citizen.Wait(250)		
	loadanimdict('mp_arrest_paired')		
	TaskPlayAnim(GetPlayerPed(-1), 'mp_arrest_paired', 'cop_p2_back_right', 8.0, -8,3750, 2, 0, 0, 0, 0)		
	Citizen.Wait(3000)		
end) 		
RegisterNetEvent('esx_statejob:douncuffing')		
AddEventHandler('esx_statejob:douncuffing', function()		
	Citizen.Wait(250)		
	loadanimdict('mp_arresting')		
...
RegisterNetEvent('esx_statejob:getuncuffed')		
AddEventHandler('esx_statejob:getuncuffed', function(playerheading, playercoords, playerlocation)		
	local x, y, z   = table.unpack(playercoords + playerlocation * 1.0)		
	SetEntityCoords(GetPlayerPed(-1), x, y, z)		
	SetEntityHeading(GetPlayerPed(-1), playerheading)		
	Citizen.Wait(250)		
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'state' then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'state' then
		...
		if IsControlJustReleased(0, 167) and not isDead and PlayerData.job and PlayerData.job.name == 'state' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'police_actions') then
			if Config.MaxInService == -1 then
...
function createBlip(id)
	...
	if not DoesBlipExist(blip) then -- Add blip and create head display on player
		...
		SetBlipColour(blip, 3) -- added by Zeref
		SetBlipShowCone(blip, true) -- added by Zeref
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_statejob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'state')
