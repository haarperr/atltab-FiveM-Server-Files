...
function OpenGearsMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'reporter_gears',
		{
			title    = 'Materiel',
			elements = {
			    {label = 'Camera TV', 			value = 'prop_tv_cam_02'},
          {label = 'Camera epaule',       value = 'prop_v_cam_01'},
		  		{label = 'Fond Vert', 			value = 'prop_ld_greenscreen_01'},
		  		{label = 'Lumieres', 			value = 'prop_worklight_03b'},
		  		{label = 'Micro pied', 			value = 'v_ilev_fos_mic'},
		  		{label = 'Ranger materiel',     value = 'clean'}
			}
		},
		function(data, menu)
			if data.current.value ~= 'clean' then
				...
				if dist < 3.0 then
					...
					ESX.ShowNotification('Vous �tes trop �loign� de votre mat�riel pour le ranger.')
				end
			end
