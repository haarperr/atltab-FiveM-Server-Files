	...
	if not playersProcessingCannabis[source] then
		...
		playersProcessingCannabis[_source] = ESX.SetTimeout(Config.Delays.WeedProcessing, function()
			...
			elseif xCannabis.count < 1 then
				...
				xPlayer.addInventoryItem('marijuana', marNbr)
				TriggerClientEvent('esx:showNotification', _source, _U('weed_processed', marNbr))
			end
...
ESX.RegisterServerCallback('esx_drugs:drugsPosition', function(source, cb)

	local weedPosition = {
		WeedField = vector3(-2597.6025, 2084.5, 78.0),
		WeedProcessing = vector3(1341.39, -1577.85, 54.44)
	}
	cb(weedPosition)
end)